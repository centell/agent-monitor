...
<p align="center">
  <img src="docs/images/ja/desktop.png" width="441"
       alt="メニューバーに 3/5 が表示され、その下にリストが開いている。承認待ち一つと入力待ち二つが一番上、シェル実行と実行中が区切り線の下にある。">
</p>
...
<img src="docs/images/en/menubar.png" width="350"
     alt="メニューバー。左に 3/5 があり、その右に入力ソース・Wi-Fi・バッテリー・時計が並ぶ。">

待っているものが無ければ `0/4` になり、**数字そのものが静かになります** — 答えるべきものが
無い間は淡く描かれ、生まれた瞬間に本来の濃さに戻ります。

<img src="docs/images/ja/menubar-clear.png" width="44"
     alt="メニューバーの数字が淡い灰色で 0/5 を示している。">
...
<img src="docs/images/ja/menu.png" width="441"
     alt="開いたメニュー。承認待ち一つと入力待ち二つが一番上、シェル実行と実行中が区切り線の下、その下にメモリ要約と「パネルを隠す」「設定…」「終了」の項目。">
...
<img src="docs/images/ja/panel.png" width="520"
     alt="デスクトップの隅に置いたパネル。ヘッダに 3/5 と最前面の印、区切り線の上に承認待ち一つと入力待ち二つ、下にシェル実行と実行中。">
...
**新しい版が出たら。** 一日に一度 GitHub に新しいリリースがあるか尋ね、あればメニューに一行
増えます — *新しい版 0.6.0 — 今すぐ入れる*。**押すまで何もダウンロードしません。** 押すと
そのリリースを取得し、届いたものが本当にこのアプリのその版か確かめ、小さなヘルパーがバンドルを
入れ替える間だけアプリは退いて、また戻ります。前の版は**新しい版が動いているのを見てから**
捨てるので、失敗した更新は空き地を残さず前の版を戻します。まるごと止めるなら **設定 → 情報**。

> このアプリは署名がないため、更新の信頼は **GitHub への HTTPS の分だけ**です — zip を手で
> 落とすときすでに預けている信頼と同じです。違いは一つ、手で落とすときは目を一度通りますが、
> 自動では通りません。
...
アプリに一切触れずコンパイルだけするなら `--no-install` — 開いていた窓が閉じません（動いているアプリは前の版のままです）。
...
- **表示** — 言語、外観（システムに従う／常に明るく／常に暗く）、行のレイアウト、どの列を
  ...
  トークン列、それぞれ別のスイッチ）、記録を取るか、メニューバーの数字にポインタを載せる
  だけでリストを開くか、更新間隔、出所を名前にどう書くか、終わった codex アプリの
  スレッドをどれだけ残すか。
  **生きたプレビュー**が実際のセッションをメニューと同じコードで描くので、見えたものがそのまま
  結果になります。

  <img src="docs/images/ja/settings-display.png" width="640"
       alt="Settings の Display タブ。言語・外観・行レイアウト・指標・トークン・記録・更新間隔・ホバーで開く・出所の表記・codex アプリのスレッドの項目が並ぶ。">
...
- **情報** — アイコンと名前、今動いている版、新しい版を知らせるかと今すぐ見るボタン
  （最後に見た時刻も）、リポジトリとライセンス。新しい版があればその行もここに出て、
  隣に **今すぐ入れる** が付きます。
