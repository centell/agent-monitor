...
public enum Addon {
    ...
    /// 애드온이 `--help` 에 덧붙이는 토막.
    ///
    /// **도움말에 없는 명령은 없는 명령과 같다.** 실제로 그랬다 — 훅이 치라고 한 `ticket`·
    /// `plan` 이 도움말에 한 줄도 없어서, 그 이름이 안 잡힌 세션이 도움말을 열어 보고
    /// 「이 도구엔 그런 기능이 없다」고 읽고 물러났다. 고장이 조용히 «기능 없음» 으로
    /// 위장된 것이다. 도움말은 막혔을 때 돌아오는 길이라, 거기가 비면 길이 없다.
    ///
    /// 그렇다고 이름을 이 저장소에 적을 수는 없다 — 적는 순간 그 글이 곧 애드온이 무엇을
    /// 하는지가 되고, 애드온이 안 꽂힌 맥에서는 **없는 명령을 있다고 말하게 된다.**
    /// 그래서 `extraCommands` 와 똑같이 자리만 낸다.
    public static var extraHelp: [String] = []
    ...
    ///
    /// 애드온이 「어떤 세션이 아직 살아 있나」를 물을 수 있게 내준다. 무엇을 그 세션에
    /// 묶어 두었는지는 **이 저장소가 알 바가 아니다** — 여기 적는 순간 그 글이 곧 애드온이
    /// 무엇을 하는지가 된다.
    ///
    /// 그 답은 잴 수 있는 값이므로 어디에도 저장하면 안 된다 — 저장하는 순간 썩기
    /// 시작하고, 죽은 세션이 살아 있는 것처럼 보이는 바로 그 거짓말이 다시 생긴다.
    ///
