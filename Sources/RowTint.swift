...
/// 띠는 상태 색 중 주황·노랑·초록을 안 쓴다 — 같은 계열이면 「급하다」로 읽힌다.
/// (시안은 셸의 청록과 가깝지만 남겨 두었다 — 넷보다 줄이면 무리가 금방 색을 나눠 쓴다.)
enum RowTint {
    ...
    /// - codex: **파랑.** 브랜드 색이 아니다 — OpenAI 브랜드는 흑백이다.
    ///
    /// 처음엔 codex 에 ChatGPT 초록 `#10A37F` 을 썼는데, **적록색약에서 둘이 같은 색이 된다.**
    /// 가독성 때문에 두 색의 밝기를 맞춰 두었더니 남은 차이가 빨강↔초록 하나뿐이었고,
    /// 그 축이 바로 적록색약이 잃는 축이다 (제2색약 모의: 둘 다 올리브 갈색, 색 차 55).
    /// 주황↔파랑은 어느 색약에서도 갈리는 짝이다 (같은 모의에서 색 차 171).
    ///
