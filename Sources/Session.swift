...
public struct Session {
    ...
    /// 지금 돌고 있는 것. 셸이 도는 줄에만 붙는다.
    ///
    /// **`reason` 과 값은 같은 데서 오는데 가는 자리가 다르다.** `reason` 은 줄에 적히므로
    /// 손이 필요한 상태에만 붙인다 — 도는 중인 줄까지 적으면 지금 할 일이 없는 줄이
    /// 목록에서 가장 시끄러워진다. 이쪽은 **마우스를 올려야 뜨는 자리**라 그 값이 없다.
    ///
    /// `pendingCall` 은 「아직 결과가 안 온 도구 호출」이다. 승인을 기다리는 호출과 지금
    /// 도는 호출은 **기록에서 같은 모양**이라 (둘 다 결과가 없다), 따로 캐낼 것이 없다.
    ///
    ...
    var running: String? {
        // **상태가 아니라 도구로 가른다.**
        //
        // 「셸이 도는가」는 `shell` 상태가 답할 것 같지만 실제로는 안 그렇다. 셸 명령을
        // 돌리는 내내 `busy` 로 적힌 세션을 봤다 — 이 앱을 짓는 세션 자신이 그랬다.
        // 거꾸로 상태는 `shell` 인데 도구는 `SendMessage` 인 세션도 있었다 (마지막 활동
        // 12분 전, 둘 중 하나가 낡은 것).
        //
        // 그러니 둘 다 요구한다. 도구 이름이 셸 도구여야 하고, 세션이 일하는 중이어야 한다.
        // 하나만 믿으면 안 뜨거나(상태만) 엉뚱한 것을 적는다(도구만).
        switch state {
        case .busy, .shell: break
        // 승인 대기는 이미 줄과 툴팁 앞머리에 그 호출이 적힌다 (`reason`). 여기 또 적으면
        // 같은 글이 두 번 나온다.
        default: return nil
        }
